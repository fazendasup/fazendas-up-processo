...
// (Planejamento — modo simples + página Hoje)
// ============================================================
...
/** Mesma regra que "Atrasados ou marcos hoje" na lista de planos (sem filtro de status). */
/**
...
export function filtrarPlanosSomenteGerminacaoPlantio(planos: any[], refDate: Date = new Date()): any[] {
  ...
  return planos.filter((p: any) => {
    if (!isPlanoOperacional(p)) return false;
    // Em produção: colheita e passos seguintes são na torre — não entram na fila só de germinação/plantio inicial.
