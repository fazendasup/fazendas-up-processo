...
export default function CapacidadePage() {
  ...
  if (data.projetoTipo === 'hidroponia') return <CapacidadeHidroponia />;
  return <CapacidadeVertical />;
}

function CapacidadeVertical() {
  const { data } = useFazenda();
...
function CapacidadeHidroponia() {
  ...
  const today = useMemo(() => new Date(), []);
  const ganttStart = useMemo(() => addDays(startOfWeek(today), weeksOffset * 7 * 4 - 14), [today, weeksOffset]);
  const ganttEnd = useMemo(() => addDays(ganttStart, 90), [ganttStart]);
  const ganttDays = 90;

  function diasFaseVariedade(varId: number | null | undefined): number {
    if (varId == null) return 30;
    const v = data.variedades.find((x) => String(x.id) === String(varId));
    return v?.diasMaturacao || 30;
  }
  function nomeVariedade(varId: number | null | undefined): string {
    if (varId == null) return 'Plantio';
    const v = data.variedades.find((x) => String(x.id) === String(varId));
    return v?.nome || 'Plantio';
  }

  const bancadaRows = useMemo(() => {
    const filtradas = faseFilter === 'all' ? bancadas : bancadas.filter((b) => b.fase === faseFilter);
    return filtradas.map((bancada) => {
      const ocupada = bancada.plantioVariedadeId != null || bancada.plantioDataEntrada != null;
      const bars: { start: Date; end: Date; label: string; color: string }[] = [];
      if (ocupada && bancada.plantioDataEntrada) {
        const entryDate = new Date(bancada.plantioDataEntrada);
        const endDate = bancada.plantioPrevisaoColheita
          ? new Date(bancada.plantioPrevisaoColheita)
          : addDays(entryDate, diasFaseVariedade(bancada.plantioVariedadeId));
        bars.push({
          start: entryDate,
          end: endDate > ganttEnd ? ganttEnd : endDate,
          label: `${nomeVariedade(bancada.plantioVariedadeId)} (${bancada.quantidadeCaixas} cx)`,
          color: FASE_COLORS[bancada.fase] || 'bg-gray-400',
        });
      }
      return { bancada, ocupada, bars };
    });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [bancadas, faseFilter, ganttEnd]);
  ...
  const projection = useMemo(() => {
    ...
    for (let w = 0; w < 8; w++) {
      ...
      const byFase: Record<string, { total: number; occupied: number; available: number }> = {};
      fases.forEach((fase) => {
        const bancadasFase = bancadas.filter((b) => b.fase === fase);
        const total = bancadasFase.length;
        let occupied = bancadasFase.filter((b) => b.plantioVariedadeId != null || b.plantioDataEntrada != null).length;
        planos.forEach((p: any) => {
          if (p.status === 'cancelado') return;
          const pStart = new Date(p.dataInicioGerminacao);
          const pEnd = new Date(p.dataColheitaPrevista);
          if (pStart <= weekEnd && pEnd >= weekStart && p.status === 'planejado') occupied++;
        });
        occupied = Math.min(occupied, total);
        byFase[fase] = { total, occupied, available: total - occupied };
      });
      weeks.push({ weekStart, weekLabel: `${formatShortDate(weekStart)} - ${formatShortDate(weekEnd)}`, byFase });
    }
    return weeks;
  }, [bancadas, planos, today]);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container py-6 space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Visualize a ocupação e planeje a capacidade das bancadas
          </p>
        </div>
        ...
        <Tabs defaultValue="gantt">
          ...
          <TabsContent value="gantt" className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => setWeeksOffset(weeksOffset - 1)} className="p-1 rounded hover:bg-muted">
                  <ChevronLeft className="w-4 h-4" />
                </button>
                <span className="text-xs text-muted-foreground px-2">
                  {formatShortDate(ganttStart)} — {formatShortDate(ganttEnd)}
            ...
            <Card>
              <CardContent className="p-0 overflow-x-auto">
                <div className="min-w-[800px]">
                  {/* Header */}
                  <div className="flex border-b bg-muted/30">
                    <div className="w-40 shrink-0 p-2 text-xs font-semibold border-r">Bancada</div>
                    <div className="flex-1 relative h-8">
                      {weekMarkers.map((m, i) => (
                        <div
                          key={i}
                      ...
                      {(() => {
                        ...
                        if (todayOffset >= 0 && todayOffset < ganttDays) {
                          return (
                            <div className="absolute top-0 h-full w-px bg-red-500 z-10" style={{ left: `${(todayOffset / ganttDays) * 100}%` }} />
                          );
                        }
                        return null;
                      })()}
                  ...
                  {/* Rows */}
                  {bancadaRows.map(({ bancada, ocupada, bars }) => (
                    <div key={bancada.id} className="flex border-b hover:bg-muted/5">
                      <div className="w-40 shrink-0 p-1.5 border-r flex items-center gap-1.5">
                        <span className="text-[11px] font-semibold truncate max-w-[5.5rem]">{bancada.nome}</span>
                        <Badge variant="outline" className="text-[8px] px-1">{FASE_LABELS[bancada.fase] || bancada.fase}</Badge>
                        {ocupada ? (
                          <span className="text-[9px] text-emerald-600 dark:text-emerald-400 font-medium">Ocupada</span>
                        ) : (
                          <span className="text-[9px] text-green-600 dark:text-green-400">Livre</span>
                        )}
                      </div>
                      <div className="flex-1 relative h-7">
                        {bars.map((bar, i) => {
                          ...
                          return (
                            <Tooltip key={i}>
                              <TooltipTrigger asChild>
                                <div className={`absolute top-1 h-5 rounded-sm ${bar.color} opacity-80 cursor-pointer`} style={style}>
                                  <span className="text-[8px] text-white px-1 truncate block leading-5">{bar.label}</span>
                                </div>
                              </TooltipTrigger>
                              <TooltipContent>
                                <p className="text-xs">{bar.label}</p>
                        ...
                        {(() => {
                          ...
                          if (todayOffset >= 0 && todayOffset < ganttDays) {
                            return (
                              <div className="absolute top-0 h-full w-px bg-red-500/30" style={{ left: `${(todayOffset / ganttDays) * 100}%` }} />
                            );
                          }
                          return null;
                        })()}
                  ...
                  {bancadaRows.length === 0 && (
                    <div className="p-8 text-center text-sm text-muted-foreground">
                      Nenhuma bancada encontrada para o filtro selecionado.
                    </div>
                  )}
                </div>
              </CardContent>
                        ...
                        <div key={bar.id} className="flex border-b hover:bg-muted/5">
                          ...
                          <div className="flex-1 relative h-7">
                            {style && (
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <div className={`absolute top-1 h-5 rounded-sm ${statusColor} opacity-70 cursor-pointer`} style={style}>
                                    <span className="text-[8px] text-white px-1 truncate block leading-5">{bar.receitaNome}</span>
                                  </div>
                                </TooltipTrigger>
                                <TooltipContent>
                                  <p className="text-xs">{bar.label}</p>
            ...
            <div className="flex flex-wrap gap-4 text-xs text-muted-foreground">
              <span className="flex items-center gap-1"><span className="w-3 h-2 rounded-sm bg-emerald-400" /> Mudas</span>
              <span className="flex items-center gap-1"><span className="w-3 h-2 rounded-sm bg-emerald-600" /> Vegetativa</span>
              <span className="flex items-center gap-1"><span className="w-3 h-2 rounded-sm bg-green-700" /> Maturação</span>
              <span className="flex items-center gap-1"><span className="w-px h-4 bg-red-500" /> Hoje</span>
            </div>
          </TabsContent>
          ...
          <TabsContent value="projecao" className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Previsão de bancadas disponíveis por fase nas próximas 8 semanas, considerando a ocupação atual e os planos de plantio.
            </p>
