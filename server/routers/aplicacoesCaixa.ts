...
export const aplicacoesCaixaRouter = router({
  listByCaixa: projectProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (!caixa) return [];
      return db.getAplicacoesByCaixaId(projetoIdFromCtx(ctx), input.caixaAguaId);
    }),
  ...
  delete: adminProjectProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await db.deleteAplicacaoCaixa(projetoIdFromCtx(ctx), input.id);
      return { success: true };
    }),
});
